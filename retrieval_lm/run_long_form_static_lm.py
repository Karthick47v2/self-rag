...
from utils import TASK_INST, PROMPT_DICT, load_special_tokens, load_jsonlines, postprocess, fix_spacing, LM_PROMPT_DICT
...
def run_step_generation_batch(model, prompt, paragraphs,  max_new_tokens,
                              ...
                              threshold=None, w_rel=1.0, w_sup=1.0, w_use=0.5, use_seqscore=False, lm_model=None, lm_tokenizer=None, raw_prompt=None):
    if paragraphs is not None:
        # aug_prompts = [prompt + "[Retrieval]" + "<paragraph>{}</paragraph>".format(
        #     paragraph["title"] + "\n" + paragraph["text"]) for paragraph in paragraphs]

        aug_prompts = [raw_prompt + "\nEvidence: {0}\n{1}".format(
            para["title"], para["text"]) for para in paragraphs]
    else:
        aug_prompts = [prompt]
    ...
    inputs = lm_tokenizer(aug_prompts, return_tensors="pt",
...
def call_model_beam_batch(prompt, model, max_new_tokens=15, ctxs=None, query=None, max_depth=5, rel_tokens=None,
                          ...
                          w_rel=1.0, w_sup=1.0, w_use=0.5, ignore_cont=False, mode="adaptive_retrieval", lm_model=None, lm_tokenizer=None, raw_prompt=None):
    special_tokens = []
    if "## Input:\n\n" in query:
    ...
    if mode == "always_retrieve":
        ...
        if "[Retrieval]" not in preds[0]:
            ...
            if threshold is None:
                ...
                do_retrieve = (
                    probs[1] / (probs[1] + probs[0]) > threshold).item()
                ...
                for node in levels[curr_depth-1]:
                    ...
                    if "[Retrieval]" in pred:
                        ...
                        preds, scores, overall_score_dict = run_step_generation_batch(
                            ...
                            threshold=threshold, w_rel=w_rel, w_sup=w_sup, w_use=w_use, lm_model=lm_model, lm_tokenizer=lm_tokenizer, raw_prompt=raw_prompt)
...
def main():
    ...
    def generate(prompt, ctxs, max_new_tokens):
        processed_prompt = PROMPT_DICT["prompt_no_input"].format_map(
            {"instruction": prompt})
        raw_prompt = LM_PROMPT_DICT["prompt_no_input"].format_map(
        ...
        return call_model_beam_batch(processed_prompt, model=model, max_new_tokens=max_new_tokens, ctxs=ctxs, query=prompt,
                                     ...
                                     w_rel=1.0, w_sup=1.0, w_use=0.5, mode=args.mode, ignore_cont=args.ignore_cont, lm_model=lm_model, lm_tokenizer=lm_tokenizer, raw_prompt=raw_prompt)
...
# python3 run_long_form_static_lm.py   --model_name selfrag/selfrag_llama2_7b   --ndocs 5 --max_new_tokens 300 --mode adaptive_retrieval --threshold 0.2   --use_grounding --use_utility --use_seqscore   --task factscore --input_file ../eval_data/factscore_unlabeled_alpaca_13b_retrieval.jsonl   --output_file temp --max_depth 7
